	...
	<div id="LayerAlertWarpper" class="LayerAlertWarpper Layer_Exp_base" >
    	<div class="layer_alert_body" >
			<div class="LayerBodySec" style="height:100%">
				<div class="popMessegeTit">
					<c:if test="${aComent eq 'codeRecV001'}">
						설정된 소관과 <span class='colorB'>검토 권한</span>을 <br>모두<span class='colorR'>해제</span>하시겠습니까?
					</c:if>
					<c:if test="${aComent eq 'codeRecV002'}">
						<span class='colorR'>1개 이상</span>의 소관과 <span class='colorB'>검토 권한</span>을 설정해주세요.
					</c:if>
        ...
        <div class="layer_alert_btn">
        	<c:if test="${alType eq 'dual'}">
        		...
        		<c:if test="${forAction eq 'saveRecConfirm' or forAction eq 'saveRecBaCon' or forAction eq 'goMetaTechDel' or forAction eq 'goMetaCtyDel' or forAction eq 'saveRecReCon' or forAction eq 'saveRecBaRe' or forAction eq 'pageLocation'}">
