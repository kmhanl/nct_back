...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
	...
	<script type="text/javascript" src="/resources/js/expert/committeeWrite.js"></script>	
    	...
    	<input type="hidden" id="committeeTechId" value="${ exData.committeeTechId }"/>
		...
		<div class="body_wrapper04">
			<div class="ComWriteTit">
				<h1 class="ViewTitle">${ exData.committeeYear }년 ${ exData.committeeTechNum }회차 <span class="TitleTech">${ exData.committeeTechName }</span> 전문위원회<%--  ${exData.committeeStatus} --%></h1>
			</div>
			...
			<div class="ComBoardBox">
				<div class="TodoListBox">
					...
					<div class="TodoLisTitBox">
						...
						<input type="hidden" id="techType" class="test" value="KTS00704" style="width:65px;" disabled="disabled">
						</span>
						</h1>
					...
					<c:if test="${exData.committeeStatus eq 'RD'}">
						<div class="ExAgendaBox" style="height:calc(100% - 133px)">
					</c:if>
					<c:if test="${exData.committeeStatus eq 'EN'}">
						<div class="ExAgendaBox" style="height:calc(100% - 133px)">
					</c:if>
					<c:if test="${exData.committeeStatus eq 'CO'}">
						<div class="ExAgendaBox" style="height:calc(100% - 133px)">
					</c:if>
					<c:if test="${exData.committeeStatus eq 'IN'}">
					...
					<div class="TotoBottom">
						...
						<div class="TodoBtnBox">
							...
							<c:if test="${exData.committeeStatus eq 'IN' }">
								<a href="/expert/committeeList.do" class="TodoBtn Exit">위원회 나가기</a>
								<a href="#none" class="TodoBtn Save" data-ex-idx="${ exData.committeeIdx }">최종 의견 제출</a>
							</c:if>
						</div>
				...
				<div class="ComBrowser">
					...
					<div id="ComBrowserBoard" class="ComBrowserBoard">
						...
						<div class="ComBrowserGuide ComBrowserPage">
							<div class="GuideBox">
								<h1 class="MainTitle">${ exData.committeeYear }년 ${ exData.committeeTechNum }회차 <span style="color:#2a80eb">${ exData.committeeTechName }</span> 전문위원회</h1>
								<div class="ComInfomation">
									<p class="InfomationL">검토 안건 :<p><p class="InfomationC"> ${ fn:length(agendaOpList) } 건</p>
								</div>
								<!-- 대면인경우 시작  -->
								...
								<c:if test="${exData.committeeStatus eq 'EN' }">
									<div class="WriteInfomation">
										<p style="font-size:19px;color:#222;text-align:center">전문위원회가 종료되었습니다.</p>
									</div>
								</c:if>
								<c:if test="${exData.committeeStatus eq 'CO' }">
									<div class="WriteInfomation">
