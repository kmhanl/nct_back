...
<script type="text/javascript" src="/resources/js/expert/exListPopup.js"></script>	
		...
		c17.1,0,31,13.9,31,31c0,17.1-13.9,31-31,31c-17.1,0-31-13.9-31-31C20,33.95,33.9,20.05,51,20.05z"/>
				...
				<ul class="BodyTopUtilTap tap_list" style="">
					...
					<li class="${param.committeeStatus eq 'SE' ? 'on' : '' }" data-status="SE"><a href="#none" class="tap_item">결과도출중${ seCnt }</a></li>
			...
			<div class="BodyList">
				<!--검색결과가 없는 경우 : 시작-->
				<c:if test="${fn:length(exLiveList) == 0 }">
				...
				<ul class="CommitListD" style="clear:both">
					<c:forEach items="${exLiveList}" var="dp" begin="${paging.startNo}" end="${paging.startNo + (paging.pageRows - 1)}" varStatus="status">
					<li class="">
						<div class="ListSection SecNo">
							${paging.totalRows - status.index}
						</div>
						<div class="ListSection CommitTech">
							<c:if test="${dp.committeeStatus eq 'RD'}">
								<p class="CommitTechIcon">전문위 예정</p>
							</c:if>
							<c:if test="${dp.committeeStatus eq 'IN'}">
								<p class="CommitTechIcon">전문위 진행중</p>
							</c:if>
							<c:if test="${dp.committeeStatus eq 'EN'}">
								<p class="CommitTechIcon">결과도출중</p>
							</c:if>
							<c:if test="${dp.committeeStatus eq 'CO'}">
								<p class="CommitTechIcon">최종검토완료</p>
							</c:if>
							
						</div>
						<%-- [${dp.committeeIdx}] ${dp.committeeStatus}--%>
						<div class="ListSection CommitTit" data-ex-idx="${dp.committeeIdx}">
							...
							<c:when test="${dp.committeeType eq 'A' }">
								...
								<c:if test="${dp.committeeDatePick eq '1'}">
									<p class="CommitIdx"><span class="ListLabel">개최일 : ${dp.committeeDatePick1 } (${ dp.committeeDateAdd1 })</span></p>	
								</c:if>
								<c:if test="${dp.committeeDatePick eq '2'}">
									<p class="CommitIdx"><span class="ListLabel">개최일 : ${dp.committeeDatePick2 } (${ dp.committeeDateAdd2 })</span></p>	
								</c:if>
								<c:if test="${dp.committeeDatePick eq '3'}">
									<p class="CommitIdx"><span class="ListLabel">개최일 : ${dp.committeeDatePick3 } (${ dp.committeeDateAdd3 })</span></p>	
								</c:if>
								
								<p class="CommitIdx"><span class="ListLabel">(${dp.committeeDatePick})자료 열람 기간 : ${dp.committeeModiSDisp } ~ ${dp.committeeModiEDisp }</span></p>
						...
						<div class="ListSection AgdCnt">
							<p class="ListLabel">의견 작성<span class="cntnum">${ dp.committeeMemberCnt } / ${dp.committeeAmg03Cnt + dp.committeeAmg04Cnt + dp.committeeAmg05Cnt + dp.committeeAmg06Cnt }</span>명</p>
							<c:if test="${dp.committeeStatus eq 'RD'}">
							...
							<c:when test="${ dp.committeeStatus eq 'CI' }">
								...
								<c:choose>
									...
									<c:when test="${ user.admGroup eq 'AMG04' }">
										<a href="#none" class="btnB goopinion" data-ex-idx="${dp.committeeIdx }">의견 작성</a>
										<a href="#none" class="btnB goopinion" data-ex-idx="${dp.committeeIdx }">계최 회수</a>
										<a href="#none" class="btnB cancleCom close-committee" data-ex-idx="${dp.committeeIdx }">전문위 종료</a>
									</c:when>
								</c:choose>
							</c:when>
							<c:when test="${ dp.committeeStatus eq 'CC' or dp.committeeStatus eq 'SE' }">
								...
								<c:when test="${ user.admGroup eq 'AMG04' }">
									<a href="#none" class="btnB goopinion" data-ex-idx="${dp.committeeIdx }">종료 회수</a>
...
<%-- 							<a href="#none" class="btnB goopinion" data-ex-idx="${dp.committeeIdx }">의견 작성</a> --%>
						...
						<c:if test="${ user.admGroup eq 'AMG04'}">
							<div class="ListSection Datecho">
								<c:if test="${dp.committeeStatus eq 'RD'}">
									<a href="javascript:cancelEx('${dp.committeeIdx}','live');" class="btn canEx" data-ex-idx="${dp.committeeIdx }">전문위 취소</a>
									<a href="javascript:openEx('${dp.committeeIdx}');" class="btn goEx" data-ex-idx="${dp.committeeIdx }">전문위 개최</a>
								</c:if>
								<c:if test="${dp.committeeStatus eq 'IN'}">
									<a href="javascript:backEx('${dp.committeeIdx}');" class="btn backEx" data-ex-idx="${dp.committeeIdx }">개최 회수</a>
									<a href="javascript:endEx('${dp.committeeIdx}');" class="btn endEx" data-ex-idx="${dp.committeeIdx }">전문위 종료</a>
								</c:if>
								<c:if test="${dp.committeeStatus eq 'EN'}">
									<a href="javascript:endbackEx('${dp.committeeIdx}');" class="btn endbackEx" data-ex-idx="${dp.committeeIdx }">종료 회수</a>
									<a href="/expert/sendcommittee.do?committeeIdx=${dp.committeeIdx}" class="btn sendEx" data-ex-idx="${dp.committeeIdx }">기술안보과 이관</a>
								</c:if>
								
								<c:if test="${dp.committeeStatus eq 'CO'}">
									<a href="javascript:returnEx('${dp.committeeIdx}');" class="btn returnEx" data-ex-idx="${dp.committeeIdx }">이관 회수</a>
								</c:if>
							</div>
						</c:if>
