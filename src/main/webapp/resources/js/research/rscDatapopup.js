...
/************ 담당자 수정 팝업 Function ************/
var ModiMem = function(){
	var wp = {};
	
	var $popup = null;
	var popupUrl = "/rsc/popup/modifyMem.do";
	...
	/** 화면에서 시작하는 경우 */
	wp.open = function(memCode){
		var rscMemCode = memCode;
		comn.popupOpen(popupUrl+"?rscMemCode="+rscMemCode, createEvent);
	};
	...
	var createEvent = function(popupElement){
		...
		var RegNotNum03 = /[^0-9]/g;
 	
	 	$popup.find(".regiNum1").off().on("keyup",function(){
			this.value = this.value.replace(RegNotNum03, '');
			var rscmackchk = $("#rscmackchk").val();
			if(rscmackchk=="Y"){
				$(".makeForm .intoOn").html("사업자등록번호와 담당자 이메일 입력후 인증키를 생성해주세요.");
				$(".makeForm .makeKeyBox").removeClass("intoOn");
				$("#rscmackchk").val("N");
			}
		});
		$popup.find(".regiNum2").off().on("keyup",function(){
			this.value = this.value.replace(RegNotNum03, '');
			var rscmackchk = $("#rscmackchk").val();
			if(rscmackchk=="Y"){
				$(".makeForm .intoOn").html("사업자등록번호와 담당자 이메일 입력후 인증키를 생성해주세요.");
		...
		$popup.find(".regiNum3").off().on("keyup",function(){
			this.value = this.value.replace(RegNotNum03, '');
			var rscmackchk = $("#rscmackchk").val();
			if(rscmackchk=="Y"){
				$(".makeForm .intoOn").html("사업자등록번호와 담당자 이메일 입력후 인증키를 생성해주세요.");
		...
		$popup.find(".memEmail").off().on("keyup",function(){
			var rscmackchk = $("#rscmackchk").val();
			if(rscmackchk=="Y"){
				$(".makeForm .intoOn").html("사업자등록번호와 담당자 이메일 입력후 인증키를 생성해주세요.");
				$(".makeForm .makeKeyBox").removeClass("intoOn");
		...
		/** 인증키 생성 */
		$popup.find(".conInfoBtn .makeKey").click(function() {
	       	var rscComCode1 = $("#regiNum1").val();
	       	var rscComCode2 = $("#regiNum2").val();
	       	var rscComCode3 = $("#regiNum3").val();
			var rscMemEmail = $("#memEmail").val();	       	
	       	var rscMemCode = $("#rscMemCode").val();
	       	
	       	if(rscComCode1==""){
	       		alert("사업자 등록번호를 첫자리를 입력해주세요.");
 				return;
	       	}
	       	if(rscComCode1.length<3){
	       		alert("사업자 등록번호를 첫자리는 3자리로 입력해주세요.");
 				return;
	       	}
	       	if(rscComCode2==""){
	       		alert("사업자 등록번호를 가운데자리를 입력해주세요.");
 				return;
	       	}
	       	if(rscComCode2.length<2){
	       		alert("사업자 등록번호를 가운데자리는 2자리로 입력해주세요.");
 				return;
	       	}
	       	if(rscComCode3==""){
	       		alert("사업자 등록번호를 끝자리를 입력해주세요.");
 				return;
	       	}
	       	if(rscComCode3.length<5){
	       		alert("사업자 등록번호를 끝자리를 5자리로 입력해주세요.");
 				return;
	       	}
	       	if(rscMemEmail==""){
	       	...
	       	var regiNum0 = rscComCode1+"-"+rscComCode2+"-"+rscComCode3;
			var rscMakekey = regiNum0+rscMemEmail;
				...
				url : "/research/proc/rscMakeKey.do", 
			 	...
			 	success : function(data) { 
			 		if(data.resultCode=="S"){
			 			$("#rscInviKey").val(data.makekey);
			 			$(".makeForm .makeKeyBox").html(data.makekey);
			 			$(".makeForm .makeKeyBox").addClass("intoOn");
			 			$("#rscmackchk").val("Y");
			 		}
				},
			 	error : function() { 
		...
		/** 정보 수정 저장  */
		$popup.find(".layer_popup_btn .Savemodimem").click(function() {
			var rscmackchk = $("#rscmackchk").val();
			if(rscmackchk=="N"){
				alert("사업자등록번호와 담당자 이메일 입력후 인증키를 생성해주세요.");
 				return;
			}
			var rscMemCompany = $("#comName").val();
			var rscComCode1 = $("#regiNum1").val();
	       	var rscComCode2 = $("#regiNum2").val();
	       	var rscComCode3 = $("#regiNum3").val();
	       	var rscMemName = $("#memName").val();	    
			var rscMemEmail = $("#memEmail").val();	       	
	       	var rscMemCode = $("#rscMemCode").val();
	       	var rscInviKey = $("#rscInviKey").val();
	       	var rscMemComCode = rscComCode1+"-"+rscComCode2+"-"+rscComCode3;
	       	console.log(rscMemCompany+":"+rscMemComCode+":"+rscMemName+":"+rscMemEmail+":"+rscMemCode);
	       	
	       	$.ajax({
				type : "POST", 
				url : "/rsc/proc/modiCom.do", 
			 	data : {
			 		rscMemCompany : rscMemCompany,
			 		rscMemComCode : rscMemComCode,
			 		rscMemName : rscMemName,
			 		rscMemEmail : rscMemEmail,
			 		rscInviKey : rscInviKey,
			 		rscMemCode: rscMemCode,
			 	}, 
			 	dataType : "json",
			 	success : function(data) { 
			 		if(data.resultCode=="S"){
			 			alert("대상기업 초대 정보가 수정 되었습니다.");
			 			location.reload();
			 		}
				},
			 	error : function() { 
