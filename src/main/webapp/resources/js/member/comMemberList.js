...
$(document).ready(function(){
	...
	var cpnStat = $("#cpnStat").val(); // 상단 탭 별 분류
	var cpnStatURLParam = $("#cpnStat").val(); // URL Param rcptStatus
	var pageRows = 10; // 한 페이지에 조회할 데이터 수
	var pagePath = "/member/commemList";
	...
	if (cpnStatURLParam && cpnStatURLParam != "null" &&
		cpnStatURLParam != "200" && cpnStatURLParam != "300" && cpnStatURLParam != "400") {
	...
	$(".nctConList .CommitTit ").on("click",function(){
		var admId = $(this).data("adm-id");
		//alert("admId===>" + admId);
		//MemberViewPopup.open(admId);
		CompanyViewPopup.open(admId);
	});
	...
	function getAdminMemberList(pageNo, pageRows) {
		...
		if (cpnStat != "" && cpnStat != null) {
			href += "&cpnStat="+cpnStat;
		}
		location.href = href;
	}
