...
/************ 소관과 > 기술 안보과 이관 팝업 ()************/
var sendCommitteeAdminPopup = function(){
	var wp = {};
	
	var $popup = null;
	var popupUrl = "/expert/popup/send/CommitteeAdminPopup.do";
	...
	wp.open = function(exIdx){
		popupUrl = '/expert/popup//send/CommitteeAdminPopup.do?committeeIdx='+exIdx;
	...
	var createEvent = function(popupElement){
		...
		$popup.find("div.layer_popup_btn  a.SendSave").on("click",function(){
		...
		$popup.find("div.layer_popup_btn a.btn_AgreeCommite").off().on("click",function(){
			...
			if(AgreeOnYN=="N"){
				...
				$.ajax({
		        	...
		        	url: "/expert/proc/commit/opinion/agreeWriteSave.do",
					dataType: "json",
					contentType: 'application/json;charset=UTF-8',
					success : function(data) {
					...
					success : function(data) {
						...
						$.ajax({
							...
							success : function(data) {
								...
								location.href = "/expert/expMemList.do";
							},
							error: function(e) {
								alert("ERROR ");
							}
...
var CommitteeWritePopup = function(){
	...
	wp.saveCommittee = function() {
		console.log(wp.commitInfo);
		console.log(wp.commitAgendas);
...
//			ComTechType 
		...
		$popup.find(".layer_popup_btn .btn_save_3003").off().on("click",function(event){
			...
			case "A":
				...
				wp.commitInfo.committeeConfirmYn = "N";
				wp.commitInfo.committeeStatus = "DP";
				break;
			case "B":
				// 서면
				if (startDate == "" || endDate == "") {
				...
				wp.commitInfo.committeeConfirmYn = "Y";
				wp.commitInfo.committeeStatus = "RD";
			...
			//회차 정보
			var committee_num = $("#committee_num").val();
			var committee_tech_num = $("#committee_tech_num").val();
			if(committee_num=="0"||committee_num==""){
				alert("전문위원회 통합 회차를 입력해주세요.");
				return;
			}
			if(committee_tech_num=="0"||committee_tech_num==""){
				alert("전문위원회 분야별 회차를 입력해주세요.");
				return;
			}
			
			
			wp.commitInfo.committeeNum = committee_num;
			wp.commitInfo.committeeTechNum = committee_tech_num;
			// 전문위원회 안건
			$popup.find("#sortable2 li.agenda_list_item").each(function(idx,agd) {
				wp.commitAgendas.push($(agd).data("agd-code"));
				wp.commitInfo.committeeAgdCnt++;
